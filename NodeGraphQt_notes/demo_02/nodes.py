...
from .port_visuals import (
    flow_input_color,
    flow_output_color,
    make_data_port_painter,
    paint_flow_input_port,
    paint_flow_output_port,
)
    ...
    def __init__(self) -> None:
        ...
        for spec in self.flow_input_specs():
            self.add_input(
                ...
                color=flow_input_color(),
                painter_func=paint_flow_input_port,
            )
        for spec in self.data_input_specs():
            self.add_input(
                ...
                painter_func=make_data_port_painter(spec.data_type),
            )
        for spec in self.flow_output_specs():
            self.add_output(
                ...
                display_name=spec.display_name,
                color=flow_output_color(),
                painter_func=paint_flow_output_port,
            )
...
class PythonSnippetNode(GeneralNode):
    """LabVIEW-style formula node: small script with wired inputs and ``result`` output."""

    NODE_NAME = "Python 代码"
    DISPLAY_NAME_ZH = "Python 代码块"
    DISPLAY_NAME_EN = "Python Snippet"
    DESCRIPTION_ZH = (
        "执行自定义 Python 片段。连线优先：输入1/输入2 来自数据端口，否则用属性默认值。"
        "命名空间含 v1、v2、variables、sessions、api_instances、log(msg)、result；"
        "请将计算结果赋给 result 以输出到下游。"
    )
    DESCRIPTION_EN = (
        "Runs a custom Python snippet. Wired v1/v2 override property fallbacks. "
        "Namespace: v1, v2, variables, sessions, api_instances, log(msg), result. "
        "Assign to result for downstream data output."
    )
    ICON_NAME = "code_snippet"
    COLOR = (55, 65, 120)
    DATA_INPUT_SPECS = (
        DataPortSpec("v1", "输入1", "any", "数据输入 1（可选）", required=False),
        DataPortSpec("v2", "输入2", "any", "数据输入 2（可选）", required=False),
    )
    DATA_OUTPUT_SPECS = (
        DataPortSpec("result", "结果", "any", "对应代码中的 result 变量", multi_connection=True),
    )
    FIELD_SPECS = (
        text_field(
            "v1",
            ...
            "Input1 prop",
            "未连线时作为 v1。",
            "Fallback for v1 when port unwired.",
        ),
        text_field(
            "v2",
            "",
            ...
            "Input2 prop",
            "未连线时作为 v2。",
            "Fallback for v2 when port unwired.",
        ),
        text_field(
            "code",
            (
                "# v1, v2: inputs\n# variables, sessions, api_instances: flow context\n"
                "# log('msg'): runtime log\nresult = v1\n"
            ),
            "Python 代码",
            "Python code",
            "exec 执行；请设置 result。",
            "Executed via exec; set result for output.",
            required=True,
            multiline=True,
        ),
    )
    ...
    def execute(self, context: Any) -> RuntimeEvent:
        v1 = self.resolve_data_input_value(context, "v1", self.field_value("v1"))
        v2 = self.resolve_data_input_value(context, "v2", self.field_value("v2"))
        code = self.field_value("code")
        if not str(code).strip():
            raise RuntimeError("Python 代码块为空，请填写 code 或删除该节点。")
        ns: dict[str, Any] = {
            "v1": v1,
            "v2": v2,
            "variables": context.variables,
            "sessions": context.sessions,
            "api_instances": context.api_instances,
            "log": lambda m: context.log("run", str(m)),
            "result": None,
        }
        try:
            exec(compile(code, f"<{self.name()}>", "exec"), {"__builtins__": __builtins__}, ns)
        except Exception as exc:
            raise RuntimeError(f"Python 代码节点执行失败: {exc}") from exc
        out = ns.get("result")
        context.set_port_value(self.id, "result", out)
        context.last_result = out
        return RuntimeEvent(
            summary="python_snippet",
            payload={"result": out},
            next_flow_key=FLOW_OUT,
            output_values={"result": out},
        )

    def emit_python(self, export_context: Any) -> list[str]:
        code = self.field_value("code")
        s1 = self.connected_data_source("v1")
        s2 = self.connected_data_source("v2")
        fb1 = self.field_value("v1")
        fb2 = self.field_value("v2")
        sid = self.id.replace(":", "_").replace(".", "_")
        return [
            f"_snippet_{sid} = {code!r}",
            f"_v1_{sid} = _read_input_value(context, {repr((s1[0].id, s1[1])) if s1 else None}, {fb1!r})",
            f"_v2_{sid} = _read_input_value(context, {repr((s2[0].id, s2[1])) if s2 else None}, {fb2!r})",
            (
                f"_ns_{sid} = {{'v1': _v1_{sid}, 'v2': _v2_{sid}, 'variables': context['variables'], "
                f"'sessions': context['sessions'], 'api_instances': context.get('api_instances', {{}}), "
                f"'log': (lambda _m: None), 'result': None}}"
            ),
            f"exec(compile(_snippet_{sid}, 'snippet_{sid}', 'exec'), {{'__builtins__': __builtins__}}, _ns_{sid})",
            f"_set_output_value(context, {self.id!r}, 'result', _ns_{sid}.get('result'))",
            f"context['last_result'] = _ns_{sid}.get('result')",
            f"return {FLOW_OUT!r}",
        ]
