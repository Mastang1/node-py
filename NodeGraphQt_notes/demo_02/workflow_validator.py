...
from .nodes import PythonSnippetNode, RaiseErrorNode, ReturnNode, WorkflowNode
...
class WorkflowValidator:
    ...
    def validate(self) -> tuple[list[ValidationMessage], FlowAnalysis | None]:
        ...
        for index, node in enumerate(analysis.ordered_nodes):
            ...
            if isinstance(node, PythonSnippetNode) and not str(node.field_value("code")).strip():
                messages.append(ValidationMessage("error", f"Python 代码块节点 {node.name()} 未填写可执行代码。"))
