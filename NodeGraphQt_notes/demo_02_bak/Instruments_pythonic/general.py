...
def delay(seconds: float) -> None:
    ...
    print(f"[General] delay -> {seconds:.3f}s")
    time.sleep(seconds)


def set_variable(variables: dict[str, Any], name: str, value: Any) -> Any:
    variables[str(name)] = value
    print(f"[General] set_variable {name} -> {value!r}")
    return value


def return_value(value: Any) -> Any:
    print(f"[General] return -> {value!r}")
    return value


def raise_error(message: str) -> None:
    print(f"[General] raise_error -> {message}")
    raise RuntimeError(str(message))


def comment(message: str) -> None:
    print(f"[General] comment -> {message}")
...
class GeneralFlowApi:
    ...
    def if_branch(self, condition: bool, true_value: str, false_value: str = "") -> list[Any]:
        ...
        print(f"[GeneralFlowApi] if_branch -> selected={selected!r} matched={bool(condition)}")
    ...
    def elif_branch(self, previous_matched: bool, condition: bool, value: str) -> list[Any]:
        ...
        print(f"[GeneralFlowApi] elif_branch -> selected={selected!r} matched={matched}")
    ...
    def else_branch(self, previous_matched: bool, value: str) -> list[Any]:
        ...
        print(f"[GeneralFlowApi] else_branch -> selected={selected!r}")
        return [selected]
    ...
    def while_loop(self, condition: bool, max_iterations: int = 10) -> list[Any]:
        ...
        while bool(condition) and iterations < int(max_iterations):
            ...
            if iterations >= int(max_iterations):
                break
        print(f"[GeneralFlowApi] while_loop -> {iterations}")
        return [iterations]
    ...
    def for_range(self, value: int) -> list[Any]:
        ...
        print(f"[GeneralFlowApi] for_range -> {items}")
        return [str(items)]
    ...
    def blocking_delay_loop(self, seconds: float, loops: int = 1) -> list[Any]:
        ...
        for _ in range(loops_value):
            ...
            total += seconds_value
        print(f"[GeneralFlowApi] blocking_delay_loop -> {total}")
        return [total]
    ...
    def terminate_flow(self, message: str = "") -> list[Any]:
        ...
        print(f"[GeneralFlowApi] terminate_flow -> {final_message}")
