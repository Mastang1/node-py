...
class Demo02Window(QtWidgets.QMainWindow):
    ...
    def _build_widgets(self) -> None:
        ...
        self.section_properties = CollapsibleSection()
        self.section_properties.add_widget(self.property_editor)

        self.section_runtime = CollapsibleSection()
        self.section_runtime.add_widget(self.runtime_text)

        self.section_validation = CollapsibleSection()
        self.section_validation.add_widget(self.validation_text)

        self.section_preview = CollapsibleSection()
        self.section_preview.add_widget(self.preview_text)
        ...
        self.section_debug = CollapsibleSection()
        self.section_debug.add_widget(self.debug_code_text)

        self.right_panel = QtWidgets.QWidget()
        right_layout = QtWidgets.QVBoxLayout(self.right_panel)
        right_layout.setContentsMargins(0, 0, 0, 0)
        right_layout.setSpacing(8)
        right_layout.addWidget(self.section_properties)
        right_layout.addWidget(self.section_runtime)
        right_layout.addWidget(self.section_debug)
        right_layout.addWidget(self.section_validation)
        right_layout.addWidget(self.section_preview)
    ...
    def _build_toolbar(self) -> None:
        ...
        self._update_debug_action_states()
    ...
    def retranslate_ui(self) -> None:
        ...
        self.section_properties.set_title(self.t("section_properties"))
        self.section_runtime.set_title(self.t("section_runtime"))
        self.section_validation.set_title(self.t("section_validation"))
        self.section_preview.set_title(self.t("section_preview"))
        self.section_debug.set_title(self.t("section_debug"))
        self.debug_code_text.setPlaceholderText(self.t("debug_hint"))
    ...
    def _update_debug_action_states(self) -> None:
        dbg_busy = self._debug_worker is not None and self._debug_worker.isRunning()
        self.actions["start_debug"].setEnabled(not dbg_busy)
        self.actions["run_flow"].setEnabled(not dbg_busy)
    ...
    def _on_debug_run_finished(self, result: Any) -> None:
        ...
        if result is not None:
            ...
            self.log("run", f"[调试] 变量: {pretty_json(result.context.variables)}")
        else:
    ...
    def start_debug_flow(self) -> None:
        ...
        worker.run_finished.connect(self._on_debug_run_finished)
        worker.run_failed.connect(self._on_debug_run_failed)
    ...
    def load_instrument_api_nodes(self) -> None:
        ...
        self.log(
            "all",
            f"仪器 API：本次新注册 {new_count} 个节点类型，扫描到 {discovered} 个带 JSON 元数据的方法，资源树共 {api_node_template_count()} 个 API 模板。",
        )
    ...
    def run_flow(self) -> Any:
        ...
        runtime = WorkflowRuntime(
            ...
            node_state_callback=self.on_runtime_node_state,
        )
        result = runtime.execute()
        self.update_runtime_panel()
        self.log("run", f"流程执行完成，返回值: {result.context.return_value!r}")
        self.log("run", f"变量上下文: {pretty_json(result.context.variables)}")
        return result
